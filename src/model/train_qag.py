...
from model_qag import QAGModel
...
class QAGDataModule(pl.LightningDataModule):
    ...
    def setup(self):
        self.train_dataset = QAGDataset(self.train_df, self.tokenizer, self.max_len_input, self.max_len_output)
        self.validation_dataset = QAGDataset(self.val_df, self.tokenizer, self.max_len_input, self.max_len_output)
        self.test_dataset = QAGDataset(self.test_df, self.tokenizer, self.max_len_input, self.max_len_output)
...
def run(args):
    ...
    model = QAGModel(params, t5_model, t5_tokenizer)
    ...
    data_module = QAGDataModule(params, t5_tokenizer, train_df, validation_df, test_df)
    data_module.setup()
    ...
    print ("Saving model...")
    save_path_model = '../../model/qag/'
    save_path_tokenizer = '../../tokenizer/qag/'
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dmn', '--dir_model_name', type=str, metavar='', default="qag", required=False, help='Directory model name.')
    ...
    parser.add_argument('-trp','--train_df_path', type=str, metavar='', default="../../data/squad_vi/raw/dataframe/df_train_qag_vi.pkl", required=False, help='Train dataframe path.')
    parser.add_argument('-vp','--validation_df_path', type=str, metavar='', default="../../data/squad_vi/raw/dataframe/df_validation_qag_vi.pkl", required=False, help='Validation dataframe path.')
    parser.add_argument('-tp','--test_df_path', type=str, metavar='', default="../../data/squad_vi/raw/dataframe/df_test_qag_vi.pkl", required=False, help='Test dataframe path.')
